...
# This script separates root and user-level installation steps for better security and permissions management.
...
run_as_root() {
    ...
    APT_PACKAGES="tor ufw gospider hakrawler gobuster ruby-dev ffuf seclists youtubedl-gui filezilla libreoffice httrack webhttrack sherlock eyewitness sublist3r photon recon-ng python3-venv python3-pip jq pipx snapd mediainfo-gui kali-tools-crypto-stego neovim cargo asciinema redis-tools finalrecon ugrep bloodhound bing-ip2hosts golang libxcb-cursor0 mat2 gallery-dl libimage-exiftool-perl stegosuite exifprobe ruby-bundler mpg123 thunderbird sqlite3 python3-lxml exiflooter flowblade dumpsterdiver npm freerdp2-x11 ftp smbclient mongo-tools villain resolvconf"
...
# --- User-level Setup ---
run_as_user() {
    log "Starting user-level setup for user: $(whoami)..."
...
# --- Main Script Logic ---
if [ "$EUID" -ne 0 ]; then
    log "This script must be run with sudo. It will automatically switch to the correct user for non-root tasks."
    sudo "$0" # Rerun the script with sudo
    exit
fi

# The script is now running as root. We will save the original user's name.
ORIGINAL_USER=$(logname)

# Run root-level tasks
run_as_root

# Switch to the original user and run user-level tasks
log "Switching to user '$ORIGINAL_USER' for user-level installations..."
su -c "bash -c '$(declare -f run_as_user); run_as_user'" "$ORIGINAL_USER"
